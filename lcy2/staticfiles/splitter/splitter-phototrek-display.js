  ...
  function load_pics(){
    $.ajax({
      ...
      success: function (data) {
        ...
        $('#info_panel').append('<div id="back_to_album" class="pull-right text-right"><button class="btn btn-primary" type="button">Back to Albums</button></div>');
        $('#back_to_album button').css({bottom: '-50px', opacity: 0});
        $('#back_to_album button').animate({
            'bottom': 0,
            'opacity': 1,
        }, 250);
